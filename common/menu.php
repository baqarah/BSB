<div class="menu-list" id="menu">
    <div id="menu-header" >
        <!-- <p class="tytul"> Whatcha need?</p2>-->
        <div id="menu">
            <a href="/common/bingo.php" class="menu-button" style="background-color: #73bec8">
                <img src="/common/menu/bingo.png"  class="menu-pic">
                <span class="menu-text">Bingo jest tutaj</span>
            </a>

            <a href="/user.php"  class="menu-button" style="background-color: #e4ba3f" >
                <img src="/common/menu/user.png" class="menu-pic" >
                <span class="menu-text" >User</span>
            </a>
            <a href="/event.php"  class="menu-button" style="background-color: #cb732b">
                <img src="/common/menu/event.png" class="menu-pic" >
                <span class="menu-text" >Event</span>
            </a>
            <a href="/hasla.php"  class="menu-button" style="background-color: #89bc62">
                <img src="/common/menu/hasla.png" class="menu-pic" >
                <span class="menu-text" >Hasla</span>
            </a>
            <a href="https://www.google.com"  class="menu-button" style="background-color: #c384c5">
                
                <span class="menu-text" >Google</span>
            </a>
            <a href="https://en.wikipedia.org/wiki/Bingo_(U.S.)"  class="menu-button" style="background-color: #c85977">
                <img src="/common/menu/help.png" class="menu-pic" >
                <span class="menu-text" >Help</span>
            </a>
        </div>
    </div>
</div>
